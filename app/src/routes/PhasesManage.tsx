...
export default function PhasesManage() {
  const t = useT();
  const { user } = useAuth();
  const { phases, loading, error } = usePhases(Boolean(user));
  ...
  useEffect(() => {
    if (!user) return;
    seedPhasesIfEmpty(user.uid).catch((e) => console.error("seed phases failed", e));
  }, [user]);
  ...
  async function handleAdd() {
    ...
    setBusy(true);
    try {
      await createPhase(newLabel, user.uid);
      setNewLabel("");
  ...
  async function handleDelete(phase: Phase) {
    const ok = window.confirm(t("phases.confirmDelete"));
    if (!ok) return;
    await deletePhase(phase.id);
  }

  return (
    <section className="mx-auto max-w-xl px-4 py-4" aria-labelledby="ph-heading">
      <h2 id="ph-heading" className="mb-2 text-xl font-semibold tracking-tight text-ink">
        {t("phases.pageTitle")}
      </h2>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={t("phases.labelPlaceholder")}
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <ul className="divide-y divide-line rounded-md bg-surface ring-1 ring-line">
            {phases.map((ph) => (
              <PhaseRow key={ph.id} phase={ph} onDelete={() => handleDelete(ph)} />
            ))}
          </ul>
        )}
      </div>
...
function PhaseRow({ phase, onDelete }: { phase: Phase; onDelete: () => void }) {
  ...
  const [draft, setDraft] = useState(phase.label);
  const [saving, setSaving] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (editing) {
      setDraft(phase.label);
  ...
  async function commit() {
    ...
    setSaving(true);
    try {
      await renamePhase(phase.id, trimmed);
      setEditing(false);
    } catch (e) {
      console.error(e);
    } finally {
