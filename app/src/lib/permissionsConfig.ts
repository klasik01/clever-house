...
export const PERMISSIONS: Record<ActionKey, PermissionRule> = {
  ...
  "budget.settings.write": {
    roles: ["OWNER"],
    description: "Upravit budget settings (limit hypotéky, banka, datum schválení).",
    rulesAt: "firestore.rules → match /budget_settings/{settingsId}",
  },
  "budget.drawdown.write": {
    roles: ["OWNER"],
  ...
  "budget.category.write": {
    roles: ["OWNER"],
    description: "CRUD OWNER-managed seznamu rozpočtových kategorií.",
    rulesAt: "firestore.rules → match /budget_categories/{categoryId}",
  },
};
