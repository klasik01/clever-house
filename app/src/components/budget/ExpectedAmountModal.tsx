...
import { setExpectedAmount } from "@/lib/budget/sections";
...
export default function ExpectedAmountModal({
  open,
  section,
  onClose,
  onSaved,
  ...
  useEffect(() => {
    if (open) {
      setCastkaInput(
        section.expectedAmountCzk ? String(section.expectedAmountCzk) : "",
      );
      setNote("");
      setError(null);
      setSubmitting(false);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!Number.isFinite(castka) || castka < 0) {
      setError(t("budget.expected.errorAmount"));
      return;
    }
    if (!note.trim() || note.trim().length < 3) {
      setError(t("budget.expected.errorNote"));
      return;
    }
    ...
    try {
      await setExpectedAmount(
        ...
        note,
        user.uid,
        section.expectedAmountCzk ?? null,
      );
      onSaved?.();
      onClose();
    } catch (err) {
  ...
  const history = section.expectedHistory ?? [];
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("budget.expected.title")}
    >
      <div className="w-full max-w-md max-h-full flex flex-col overflow-hidden rounded-xl bg-bg shadow-xl ring-1 ring-line">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <p className="text-sm text-ink-muted">
            {t("budget.expected.intro")}
          </p>

          <label className="block text-sm font-medium text-ink">
            {t("budget.expected.amountLabel")}
          ...
          <label className="block text-sm font-medium text-ink">
            ...
            <textarea
              ...
              disabled={submitting}
              placeholder={t("budget.expected.notePlaceholder")}
