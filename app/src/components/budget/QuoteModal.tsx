...
import { createQuote, updateQuote } from "@/lib/budget/quotes";
...
export default function QuoteModal({
  ...
  useEffect(() => {
    if (open) {
      setCastkaInput(quote ? String(quote.castka) : "");
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!Number.isFinite(castka) || castka <= 0) {
      setError(t("budget.quote.errorAmount"));
      return;
    }
    ...
    try {
      if (mode === "create") {
        const id = await createQuote(
          sectionId,
          { castka, supplier, note },
          user.uid,
        );
        onSaved?.(id);
      } else if (quote) {
        await updateQuote(sectionId, quote.id, { castka, supplier, note });
        onSaved?.(quote.id);
      }
      onClose();
    } catch (err) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={mode === "create" ? t("budget.quote.newTitle") : t("budget.quote.editTitle")}
    >
      <div className="w-full max-w-md overflow-hidden rounded-xl bg-bg shadow-xl ring-1 ring-line">
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <h2 className="text-base font-semibold text-ink">
            {mode === "create"
              ? t("budget.quote.newTitle")
              : t("budget.quote.editTitle")}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-4 py-4">
          <p className="text-xs text-ink-muted">{t("budget.quote.intro")}</p>

          <label className="block text-sm font-medium text-ink">
            {t("budget.quote.amountLabel")}
          ...
          <label className="block text-sm font-medium text-ink">
            ...
            <input
              ...
              disabled={submitting}
              placeholder={t("budget.quote.supplierPlaceholder")}
          ...
          <label className="block text-sm font-medium text-ink">
            ...
            <textarea
              ...
              disabled={submitting}
              placeholder={t("budget.quote.notePlaceholder")}
